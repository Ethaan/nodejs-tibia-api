  ...
  _createClass(TibiaAPI, [{
    ...
    value: function getOnlinePlayers(worldName) {
      ...
      return new Promise(function (resolve, reject) {
        var worldNameToUse = worldName ? worldName : _this.world;
        if (!worldNameToUse) {
    ...
    value: function getGuildInformation(guildNameOrUrl) {
      if (!guildNameOrUrl) {
        console.warn('Guild Name or url is needed');
        return;
      };
      var guildUrl = guildNameOrUrl;
      var baseGuildsUrl = 'https://secure.tibia.com/community/?subtopic=guilds&page=view&GuildName=';
      var isByUrl = (0, _stringUtils.isUrl)(guildNameOrUrl);
      if (!isByUrl) {
        guildUrl = '' + baseGuildsUrl + guildNameOrUrl.replace(/ /g, '+');
      };
      return new Promise(function (resolve, reject) {
        (0, _utils.getGuildInformationByUrl)(guildUrl).then(function (result) {
          resolve(result);
        }).catch(function (error) {
          return reject(error);
        });
...
var tibia = new TibiaAPI({ worldName: 'Zanera' });

tibia.getGuildInformation('Last Resort').then(function (result) {
  console.log(result);
}).catch(function (error) {
  return console.log(error);
});
